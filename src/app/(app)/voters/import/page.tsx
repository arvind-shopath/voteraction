...
export default function ImportVotersPage() {
    ...
    const fetchQueue = async () => {
        try {
            const res = await fetch('/api/voters/import/queue');
            if (res.ok) {
    ...
    const handleUpdateJob = async () => {
        ...
        setIsUpdating(true);
        try {
            const res = await fetch(`/api/voters/import/queue/job?id=${editingJob.id}`, {
                method: 'PATCH',
    ...
    const handleDeleteJob = async (job: any) => {
        ...
        try {
            const res = await fetch(`/api/voters/import/queue/job?id=${job.id}`, { method: 'DELETE' });
            if (res.ok) {
                const data = await res.json();
                fetchQueue();
            } else {
                                ...
                                {filteredQueue.map((job) => (
                                    <div key={job.id} style={{
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '13px' }}>
                                            ...
                                            <div>
                                                <div style={{ color: '#94A3B8', fontSize: '11px', fontWeight: '600' }}>VILLAGE/WARD</div>
                                                <div style={{ fontWeight: '600', color: '#475569', marginTop: '4px' }}>{job.boothName || '---'}</div>
                                            </div>
                                        </div>
                                ...
                                <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                                    <thead style={{ background: '#F1F5F9', color: '#475569', fontWeight: '700', textAlign: 'left' }}>
                                        <tr>
                                            ...
                                            <th style={{ padding: '12px 24px' }}>गांव/वार्ड (Village/Ward)</th>
